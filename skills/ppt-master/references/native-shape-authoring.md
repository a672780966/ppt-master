...
**Preferred for one already-selected preset — `shape.create`**: `python3 ${SKILL_DIR}/scripts/semantic_tools.py shape.create --input '{"id":"p03-growth-arrow","preset":"rightArrow","frame":{"x":160,"y":210,"width":320,"height":112},"fill":"#2563EB","stroke":"none"}'` returns `{ok, artifact_id, native_type, svg_fragment}` for the exact same underlying fragment `render` below produces — same insertion rule (insert `svg_fragment` unchanged, never redirect into `svg_output/`), same `UNKNOWN_PRESET`/`INVALID_FRAME` failure instead of a raw traceback ([`semantic_tools.md`](../scripts/docs/semantic_tools.md)). Use the raw `render`/`render-batch` calls below only for validating/looking up several objects at once (`describe`, `list`) or a connector preset batch — `shape.create` is single-object only, and Boolean composition still requires `shape_boolean_svg.py` (no semantic tool wraps it).
